<footer>
  <div class="container">
    <div class="row">
      <div class="col-lg-8 socialmedia__icons">
            <div class="col-sm-3">
                <i class="fa fa-facebook" aria-hidden="true"><span>facebook</span></i>
            </div>
            <div class="col-sm-3">
              <i class="fa fa-twitter" aria-hidden="true"><span>twitter</span></i>
            </div>
            <div class="col-sm-3">
              <i class="fa fa-instagram" aria-hidden="true"><span>instagram</span></i>
            </div>
            <div class="col-sm-3">
              <i class="fa fa-pinterest" aria-hidden="true"><span>pinterest</span></i>
            </div>
      </div>
    </div>
    <hr>
  <div class="row">
    <div class="col-lg-12 info">
      <div class="col-sm-4 homebase">
        <h1>homebase</h1>
        <p><i class="fa fa-phone" aria-hidden="true"></i>733-333-333</p>
        <p><i class="fa fa-envelope" aria-hidden="true"></i>teammahout@gmail.com<p>
        <p>
          &copy; <?php echo date("Y"); ?> team mahout.
        </p>
        <p> All rights reserved</p>
      </div>
      <div class="col-sm-3 ourstory">
        <h1>our stories</h1>
        <p>
          Featured
        </p>
        <p>
          Start Ups
        </p>
        <p>
          Technology
        </p>
        <p>
          Company News
        </p>
      </div>
      <div class="col-sm-1 mahout__footer__logo">
        <img src="<?php echo get_stylesheet_directory_uri(); ?>/img/mahoutwhitelogo.png">
      </div>
    </div>

  </div>
  </div>








<?php wp_footer(); ?>


</footer>
